...
def fruitcrawl():
    ...
    for fruit in fruit_list_with_modification:
...
def calculate_page_rank(matrix, beta=0.85, eps=1.0e-8, iterations=100000):
    ...
    A = beta * matrix + (1 - beta) / n * np.ones([n, n])
...
def combined_distance(centeroid, data_point):
    euclidean = calculate_euclidean_distance(centeroid[:6], data_point[:6])
    cosine = calculate_cosine_similarity(centeroid[6:], data_point[6:])

    return euclidean + (1 - cosine)  


def calculate_kmeans(feature_vectors:list, k:int, iteration:int, section:str ):
    ...
    for i in range(iteration):
        ...
        for n in range(num_of_data_points):
                #This part is changed from the original algorithm,  we choose diffrent distance algorithm based on the cenrio
                if section in ['d','e']:
                ...
                if section in ['g']:
                    distances = [combined_distance(centroid, feature_vectors[n]) for centroid in centroids]
                    data_point_groups[n] = np.argmin(distances)
...
def kmeans(graph:bool = True):
    ...
    data_point_groups,centroids  = calculate_kmeans(feature_vectors,4,50,'d')
    if graph:
...
def q8_e(graph:bool = True):
    ...
    seasons = {"Winter": 0,'Fall':0.25 ,"Spring": 0.75 , "Summer": 1} #valued simillarity between seasons
    ...
    feature_vectors = []
    for _ , row in DataFrame.iterrows():                            #each color get number between 0-1 according is brightness 
        feature_vectors.append(np.array([Peeling_Messiness[row[a]], colors.index(row[b])/len(colors), seasons[row[c]]])) 

    data_point_groups, _ = calculate_kmeans(feature_vectors,4,50,'e')
    if graph:
...
def q8_f(top_n_words_tdidf:dict, graph:bool = True):
    ...
    data_point_groups, _  = calculate_kmeans(feature_vectors,4,50,'f')
    if graph:
        x_point = DataFrame["Amount of Sugar"].tolist()
        y_point = DataFrame["Price"].tolist()
...
def combine_feature_vec(top_n_words_tdidf):
    ...
    for i in range(len(fruit_list_with_modification)):
        v = np.concatenate((v_kmeans[i],v_e[i],v_f[i]))
...
def q8_g(top_n_words_tdidf):
    ...
    data_point_groups, _  = calculate_kmeans(combined_feature_vec,4,50,'g')
